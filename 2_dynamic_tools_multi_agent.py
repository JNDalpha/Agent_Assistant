...
SUPERVISOR_MESSAGE_MAX_LENGTH = message_config["supervisor_message_max_length"]
...
def run():
    ...
    query = input("输入：")
    supervisor_message.append(HumanMessage(content=query)) # type: ignore
    final_output = ""
    ...
    if len(supervisor_message) > SUPERVISOR_MESSAGE_MAX_LENGTH:
        supervisor_message.pop(0)
        supervisor_message.pop(0)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print(f"total input tokens: {total_conversation_input_token}, total output tokens so far: {total_conversation_output_token}")
        if tts_flag:
